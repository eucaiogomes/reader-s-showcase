          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              {categories.map(([categoryId, category]) => {
                ...
                const hasSubs = (category as any).subs?.length > 0;
                return (
                  <button
                    key={categoryId}
                    onClick={() => selectCategory(categoryId)}
                    className={`rounded-2xl border px-4 py-2 text-sm font-semibold transition-all duration-150 whitespace-nowrap flex items-center gap-2 ${
                      isActive
          ...
          {categoryData.subs?.length && activeCategory !== 'all' ? (
            <div className="mb-4">
              <div
                ...
                style={{
                  maxHeight: activeCategory !== 'all' && categoryData.subs?.length ? '200px' : '0',
                  opacity: activeCategory !== 'all' && categoryData.subs?.length ? 1 : 0,
                  transform: activeCategory !== 'all' && categoryData.subs?.length ? 'translateY(0)' : 'translateY(-10px)',
                }}
              >
