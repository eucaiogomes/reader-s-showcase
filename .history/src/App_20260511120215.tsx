        ...
        <div className="bg-white border border-gray-200 rounded-[28px] shadow-sm p-4">
            ...
            {categories.map(([categoryId, category]) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-full border px-4 py-2 text-sm font-semibold transition-all duration-150 whitespace-nowrap ${
                    isActive
          ...
          <div
            className={`overflow-hidden transition-all duration-200 ${
              categoryData.subs?.length && activeCategory !== 'all' ? 'max-h-[220px] opacity-100' : 'max-h-0 opacity-0'
            }`}
          >
          ...
          <div className="mt-4 text-sm text-gray-500">
            {VITRINES.find((v) => v.id === activeVitrineId)?.descricao}
          </div>
        </div>
      </div>
    </div>
