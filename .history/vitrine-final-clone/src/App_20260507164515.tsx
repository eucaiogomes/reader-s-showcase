...
const CategoryDropdown = ({
  ...
  const [selectedVitrines, setSelectedVitrines] = useState<Set<string>>(new Set(['v1', 'v2']));
  ...
  const handleToggleVitrine = (vitrinaId: string) => {
    const newSet = new Set(selectedVitrines);
    if (newSet.has(vitrinaId)) {
      newSet.delete(vitrinaId);
    } else {
      newSet.add(vitrinaId);
    }
    setSelectedVitrines(newSet);
  };

  const handleSelectAll = () => {
    setSelectedVitrines(new Set(vitrinesForDisplay));
  };

  const handleClear = () => {
    setSelectedVitrines(new Set());
  };
  ...
  return (
    ...
    >
      <div className="max-w-[1600px] 2xl:max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-10 xl:px-16 py-4">
        {/* Categories */}
        <div className="mb-4">
          <div className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-3">Categoria</div>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                  activeCategory === key
                    ...
                    : 'bg-gray-100 text-gray-600 hover:bg-gray-150 border border-transparent'
                }`}
              >
                {data.label}
              </button>
        ...
        {/* Subcategories */}
        {CATEGORY_DATA[activeCategory as keyof typeof CATEGORY_DATA]?.subs?.length > 0 && (
          <div className="mb-4">
            <div className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-3">Subcategoria</div>
            <div className="flex flex-wrap gap-2">
              <button
              ...
              {CATEGORY_DATA[activeCategory as keyof typeof CATEGORY_DATA].subs.map((sub) => (
                <button
                  key={sub.id}
        ...
        <div className="border-t border-gray-200 pt-4">
          <div className="flex items-center justify-between mb-3">
            <div className="text-xs font-semibold text-gray-500 uppercase tracking-widest">Coleções disponíveis</div>
          ...
          <div className="flex flex-wrap gap-2">
            {vitrineObjects.map((vitrine) => (
              <button
                key={vitrine!.id}
                onClick={() => handleToggleVitrine(vitrine!.id)}
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all flex items-center gap-2 ${
                  selectedVitrines.has(vitrine!.id)
                    ? 'bg-brand-primary/10 text-brand-primary border border-brand-primary/20'
                    : 'bg-gray-100 text-gray-600 hover:bg-gray-150 border border-transparent'
                }`}
              >
                {selectedVitrines.has(vitrine!.id) && <Check className="w-4 h-4" />}
                {vitrine!.nome}
              </button>
            ))}
          </div>
          {selectedVitrines.size > 0 && (
            <div className="mt-3 text-xs text-gray-600 bg-gray-50 p-2 rounded-lg">
              <strong>{selectedVitrines.size} coleção(ões) ativa(s):</strong> {Array.from(selectedVitrines).map(id => VITRINES.find(v => v.id === id)?.nome).join(', ')}
            </div>
          )}
        </div>
      </div>
